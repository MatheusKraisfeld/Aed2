...
class Lista{
   ...
   //verifica se o nTemp em 'i' eh menor do que em 'j' 
   public boolean ehMenor(int i, int j){
        boolean tof = false;
        //int comp = i.compareTo(j);
        comparacao++;
        if(i < j){
   ...
   //verifica se o nTemp eh igual
   public boolean ehIgual(int i, int j){
        boolean tof = false;
        //int comp = i.compareTo(j);
        comparacao++;
        if(i == j){
    ...
    //Ordena o array utilizando o método de mergesort. 
   public void mergeSort(){
      Series[] arrayMerge = new Series[cont];
      for(int i = 0; i < cont; i++){
         arrayMerge[i] = array[i].clone();

      }
      mergeSort(arrayMerge, 0, cont - 1);
   }
 
   
   //Utiliza o método de mergesort para ordenar a lista;
   public void mergeSort(Series[]arrayMerge, int esq, int dir){
      int meio;
      if (esq < dir){
         meio = ((dir + esq) / 2);
         mergeSort(arrayMerge, esq, meio);
         mergeSort(arrayMerge, meio + 1, dir);
         merge(arrayMerge, esq, meio, dir);
      }
   }
 
    
   //Junção de dois vetores já ordenados. 
   public void merge(Series[]arrayMerge, int esq, int meio, int dir) {
      int k;
      int i = esq;
      int j = meio + 1;
            
      for(k = esq; k <= dir; k++){
         arrayMerge[k] = array[k];
      ...
      for(k = esq; k <= dir; k++){
         ...
         else if(ehMenor(arrayMerge[i].getnEp(),arrayMerge[j].getnEp())){
            array[k] = arrayMerge[i++];
            mov++;
         }
         
         else if((ehIgual(arrayMerge[i].getnEp() , arrayMerge[j].getnEp())) && compararString1(arrayMerge[i].getNome() , arrayMerge[j].getNome())){
            array[k] = arrayMerge[i++];
            mov++;
         }
...
class TP05Q11{
   public static void main(String[] args)throws Exception{
      ...
      long esq = System.currentTimeMillis();
      L.mergeSort();
      ...
      Arq.openWrite("576473_mergesort.txt", "UTF-8");
      Arq.println("Matricula: 576473\tTempo:" + (fim-esq)/1000.0 + "s\t" + "Compracoes:"+L.comparacao+"\t"+"Movimentacoes:"+L.mov); 
      Arq.close();
