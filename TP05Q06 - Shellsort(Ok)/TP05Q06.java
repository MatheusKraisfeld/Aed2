...
//retorna true se a string i for maior que a string j;
   ...
   public void shell(){
    ...
    public void insercaoPorCor(int cor, int h){
        int j;
        for(int i = h + cor; i < cont; i += h){
            Series tmp = array[i].clone();
            j = i - h;
            while(j >= 0 && ehMaior(array[j].getIdiomaOriginal(), tmp.getIdiomaOriginal(), array[j].getNome(), tmp.getNome())){
   ...
   //Compara se um objeto vem antes do outro na ordenação. Se o idioma for igual, vamos ordenar por nome; 
...
class TP05Q06{
   public static void main(String[] args)throws Exception{
      ...
      Arq.openWrite("576473_shellsort.txt", "UTF-8");
