...
class Lista{
   ...
   public Lista(int tamHeap){
      array = new Series[tamHeap];
   ...
   public void constroi(int tamHeap){
      ...
      for(i = tamHeap; i > 1 && ehMaior(array[i].getFormato(), array[i/2].getFormato(), array[i].getNome(), array[i/2].getNome()); i /= 2){
         tmp = array[i].clone();
   ...
   public void reconstroi(int tamHeap){
      ...
      int filho;
 		...
 		while(i <= (tamHeap/2)){
         if(ehMaior(array[2*i].getFormato(), array[2*i+1].getFormato(), array[2*i].getNome(), array[2*i+1].getNome()) || 2*i == tamHeap){
            filho = 2*i;
         }
         else{
            filho = 2*i+1;
         ...
         if(ehMenor(array[i].getFormato(), array[filho].getFormato(), array[i].getNome(), array[filho].getNome())){
            tmp = array[i].clone();
   ...
   public void heap(){
      ...
      //construcao do heap
      for(int tamHeap = 2; tamHeap <= cont; tamHeap++){
         constroi(tamHeap);
      }
...
class TP05Q07{
   public static void main(String[] args)throws Exception{
      ...
      Arq.openWrite("576473_heapsort.txt", "UTF-8");
