...
class Lista{
    ...
    */
    private void quicksort(int esq, int dir) {
        int i = esq, j = dir;
        String pivo = array[(dir+esq)/2].getPaisOrigem();
        String pivoNome = array[(dir+esq)/2].getNome();
        while (i <= j) {
            while (ehMenor(array[i].getPaisOrigem() , pivo , array[i].getNome() , pivoNome)){
             i++;
            }
            while (ehMaior(array[j].getPaisOrigem() , pivo , array[j].getNome() , pivoNome)){
               j--;
            }
            if (i <= j) {
...
class TP05Q08{
   public static void main(String[] args)throws Exception{
      ...
      Arq.openWrite("576473_quicksort.txt", "UTF-8");
