...
class Lista{
   ...
   //utiliza o algoritmo de selecao para ordenar o vetor;
   public void selecao(){
         for (int i = 0; i < (cont - 1); i++) {
         int menor = i;
         for (int j = (i + 1); j < cont; j++){
           
            if (compararString(array[menor].getNome() , array[j].getNome())){
               menor = j;
            }
            
         }
         
         Series tmp = array[menor];
         array[menor] = array[i];
         array[i] = tmp;
         mov++;
      }
   ...
   //metodo que verifica se tal elemento esta no array;
   public boolean pesquisaBinaria(String elemento) throws Exception{
      boolean tof = false;
      int meio;
      int inicio = 0;
      int fim = cont - 1;
      ...
      while(inicio <= fim){
         meio = (inicio + fim) / 2;
         if(stringIgual(elemento,array[meio].getNome())){
            tof = true;
            inicio = fim + 1;
         }
         else if(compararString(elemento,array[meio].getNome())){
            inicio = meio + 1;
         }
         else if(compararString1(elemento,array[meio].getNome())){
            fim = meio - 1;
         }
      }
      return tof;
...
class TP05Q02 {
   public static void main(String[] args)throws Exception {
      ...
      Arq.openWrite("576473_binaria.txt", "UTF-8");
      Arq.println("Matricula: 576473\tTempo:" + (fim-inicio)/1000.0 + "s\t" + "Compracoes:"+L.comparacao); 
      Arq.close();
