...
class TP5Q12{
   public static void main(String[] args)throws Exception{
      ...
      Arq.openWrite("576473_radixsort.txt", "UTF-8");
...
class Lista{
   ...
   public boolean stringIgual(String j , String i){
 	...
 	public int getMaior() {
       int maior = array[0].getnEp() * 1000 + array[0].getnTemp();
 
        for (int i = 0; i < cont; i++) {
         if(maior < array[i].getnEp() * 1000 + array[i].getnTemp()){
            maior = ((array[i].getnEp() * 1000 + array[i].getnTemp()));
         }
        }
       return maior;    
    }
 ...
 /**
    ...
    public void countingsort() {
      ...
      Series ordenado[] = new Series[cont];
 
      //Inicializar cada posicao do array de contagem 
      for (int i = 0; i < getMaior() + 1; i++){
	  	count[i] = 0;
	  }
 
      //Agora, o count[i] contem o numero de elemento iguais a i
      for (int i = 0; i < cont;i++){
	  	count[((array[i].getnEp() * 1000 + array[i].getnTemp()))]++;
 	  }
      ...
      for(int i = cont-1; i >= 0; i--){
	    ordenado[count[((array[i].getnEp() * 1000 + array[i].getnTemp()))] - 1] = array[i];
	    count[((array[i].getnEp() * 1000 + array[i].getnTemp()))]--;
	    mov++;
	  }
...
//utiliza o algoritmo de selecao para ordenar o vetor;
   public void selecao(){
         ...
         for (int j = (i + 1); j < cont; j++){
           if(ehMaior(((array[menor].getnEp() * 1000 + array[menor].getnTemp())), ((array[j].getnEp() * 1000 + array[j].getnTemp())), array[menor].getNome() , array[j].getNome())){
               menor = j;
           }
